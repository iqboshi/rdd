...
## 2026-03-19 | A8-LTO-代码落地与恢复训练修复
- 实验ID:
  - `ablation_aux_a8_lto_v1_q128`
  - `ablation_aux_a8_lto_v1_q128_resume_smoke`
- 目标: 在 A7-s2 固定基线上加入“可发表的结构性创新”（Leaf Topology Ordering, LTO）
- 关键配置:
  - 模型: 新增 query 级顺序头 `pred_order`（`model.py`）
  - 训练: 新增 `loss_order` 及参数
    - `w_order=0.05`
    - `order_min_dy=4`
    - `order_pair_max_dist=180`
    - `order_max_pairs=4096`
    （`losses_v5.py`, `train-v5.py`）
  - 解码: 新增 LTO 叠加排序后处理开关（`scripts/build_review_pack.py`）
  - 训练起点: 从 A7 最优 checkpoint resume
    - `outputs/ablation_aux_a7_topology_q128/exp_20260318_120451/checkpoints/best.pth`
- 结果摘要:
  - 首次启动报错: `optimizer` 参数组与新架构不匹配（旧 checkpoint 不含 order head 参数）
  - 已修复 `train-v5.py` 的 resume 逻辑:
    - 模型权重照常加载
    - `optimizer/scheduler/scaler` 不兼容时自动降级为 fresh state，不再中断
    - 启动日志会显示 `optimizer_loaded/scheduler_loaded/scaler_loaded` 状态
  - 复跑验证:
    - `Resume optimizer state incompatible... Falling back...` 为预期行为
    - `lr_scheduler.step()` 的两条 warning 为恢复阶段一次性提示，可忽略
  - 本轮训练建议: 将总轮数设为 `--epochs 95`（从 `start_epoch=45` 继续，约再跑 50 轮）
- 结论: A8-LTO 训练已进入可稳定续跑状态；等待完整训练收敛后做门控评测与盲评触发判断

## 2026-03-19 | A8-Train-Speed-Optimization（等价代码级加速）
- 实验ID:
  - `ablation_aux_a8_lto_v1_q128`
  - `a8_runtime_speedup_20260319`
- 目标: 在不改变训练任务定义和损失目标的前提下，显著缩短单 epoch 时间
- 关键配置:
  - 先做瓶颈快照（训练进行中）:
    - GPU: `97%~100%`，显存约 `7776/8188MiB`，`P0`
    - CPU: 总占用约 `11%~17%`（24线程）
    - 磁盘: 队列长度低（`<0.04`）
  - 核心优化（代码实现层，不改目标函数）:
    - `losses_v5.py`
      - 向量化 `build_order_targets_for_batch`
      - 向量化 `compute_affinity_embedding_loss`
      - 去除 `order/overlap` 中不必要 `.item()` 同步
      - 重写 `HungarianMatcher` 的 mask/dice 代价计算，避免 `QxNxHxW` 展开
    - `train-v5.py`
      - loss统计改为 GPU 端累计，epoch末再同步
      - 新增 `--tqdm_postfix_interval`（默认20）降低进度条同步开销
      - 验证阶段使用 `torch.inference_mode()`
      - 新增 runtime 开关: `--cudnn_benchmark`（默认开）、`--allow_tf32`（默认开）
      - 优化器自动尝试 `AdamW(fused=True)`（不支持自动回退）
  - 校验:
    - `python -m py_compile losses_v5.py train-v5.py` 通过
    - `D:\anaconda\envs\dl40\python.exe` 下 matcher/loss smoke 通过
- 结果摘要:
  - 用户反馈: 训练速度约 `20分钟/epoch -> 5分钟/epoch`
  - 结论与诊断一致: 原瓶颈主要在计算图实现与同步开销，而非 dataloader 参数本身
- 结论:
  - 采纳本轮提速实现为默认训练路径
  - 训练逻辑/损失定义未改变；但 `TF32/fused` 可能引入轻微数值差异（非算法变更）
