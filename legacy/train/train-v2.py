...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Freeze early Swin stages. 0 means no freeze.",
    )
...
def _swin_param_stage_id(param_name: str):
    if param_name.startswith("patch_embed"):
        return 0
    if param_name.startswith("layers_"):
        try:
            layer_idx = int(param_name.split(".")[0].split("_")[1])
            return layer_idx + 1
        except Exception:
            return None
    return None
...
def freeze_backbone_stages(model: torch.nn.Module, freeze_stages: int = 2):
    freeze_stages = int(max(0, freeze_stages))
    frozen_names = []
    trainable_names = []

    backbone_module = getattr(getattr(model, "backbone", None), "backbone", None)
    if backbone_module is None:
        print("[WARN] backbone module not found, skip freezing.")
        return frozen_names, trainable_names

    for name, param in backbone_module.named_parameters():
        stage_id = _swin_param_stage_id(name)
        if stage_id is not None and stage_id <= freeze_stages:
    ...
    print(
        f"[INFO] freeze_backbone_stages: freeze_stages={freeze_stages}, "
        f"frozen_backbone_params={len(frozen_names)}, trainable_backbone_params={len(trainable_names)}"
    )
    if len(frozen_names) > 0:
        preview = ", ".join(frozen_names[:6])
        if len(frozen_names) > 6:
            preview += ", ..."
        print(f"[INFO] Frozen backbone params preview: {preview}")
    if len(trainable_names) > 0:
        preview = ", ".join(trainable_names[:6])
        if len(trainable_names) > 6:
            preview += ", ..."
        print(f"[INFO] Trainable backbone params preview: {preview}")
...
def count_parameters(model: torch.nn.Module):
    ...
    return int(total), int(trainable), int(frozen)
...
def main():
    ...
    freeze_backbone_stages(model, freeze_stages=args.freeze_stages)
    total_params, trainable_params_count, frozen_params_count = count_parameters(model)
    print(f"[INFO] Params total: {total_params:,}")
    print(f"[INFO] Params trainable: {trainable_params_count:,}")
    print(f"[INFO] Params frozen: {frozen_params_count:,}")
