...
def parse_args():
    ...
    parser.add_argument("--num_queries", type=int, default=40)
...
def build_param_groups(model: torch.nn.Module, backbone_lr: float, head_lr: float):
    backbone_params = []
    head_params = []
    ...
    param_groups = [
        ...
        {"params": head_params, "lr": float(head_lr), "group_name": "head"},
    ]
    return param_groups
...
def main():
    ...
    print("[INFO] Backbone freeze: disabled (all stages trainable)")

    backbone_total = sum(p.numel() for p in model.backbone.parameters())
    backbone_trainable = sum(p.numel() for p in model.backbone.parameters() if p.requires_grad)
    print(f"[INFO] Backbone params trainable: {backbone_trainable:,}/{backbone_total:,}")
    ...
    for idx, group in enumerate(optimizer.param_groups):
        ...
        gname = group.get("group_name", f"group_{idx}")
        print(f"[INFO] Optimizer group {idx} ({gname}): lr={group['lr']:.6e}, params={n_params:,}")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        msg = (
            ...
            f"lr_backbone={lr_backbone:.6e}, lr_head={lr_head:.6e} | time={elapsed:.1f}s"
        )
        print(msg)
